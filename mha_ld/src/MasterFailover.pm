...
###    if ( $g_interactive == 0 ) { 
###      MHA::ProxyManager::setproxy("failover","unknown","$addr","rw","$logfile","$confile");
###    }
###    else {
###      MHA::ProxyManager::setproxy_intera("failover","unknown","$addr","rw","$confile");      
...
###      }
  ...
  ###### added by cch 20160822 sendproxy ,cancel before sendinfo #####################
  foreach my $deadserver (@dead_servers) {
    $log->info( sprintf( "Dead server: %s", $deadserver->get_hostinfo() ) );
    my $deadaddr = $deadserver->get_hostinfo();
    $deadaddr =~ /\((.*)\)/;
    $log->info("deadaddr: $1");
  ...
  $pm->run_on_finish(
    sub {
      ...
      if ( $exit_code == 0 ) {
        ...
        #### begin added by cch ###########################################
        $log->info("$target->get_hostinfo()");    
        my $target_addr=$target->get_hostinfo();
        $log->info("before  target_addr: $target_addr");
        #$target_addr =~ /\((.*)\)/; 
...
#      $latest_base_slave );
    ...
    if ( $g_interactive == 0 ) {
      $recover_slave_rc =
        recover_slaves_internal( $new_master, $master_log_file, $master_log_pos,
        $latest_base_slave, $logfile , $confile );
    }
    else {
      $recover_slave_rc =
        recover_slaves_internal( $new_master, $master_log_file, $master_log_pos,
...
sub send_report {
  ...
  if ( $mail_subject && $mail_body ) {
    $log->info( "\n\n"
      ...
      #### begin added by cch ###############################################
      my $gt=gettime;
      send_alert($gt . $mail_subject);

      my $sendmail=$g_workdir . "/sendmail.txt";
      $log->info("g_workdir: $g_workdir");
      $log->info("sendmail:  $sendmail ");
      if ( -f $sendmail ) {
         MHA::NodeUtil::drop_file_if($sendmail);  
      }
      MHA::NodeUtil::create_file_if($sendmail);

      my $str1="echo \"$mail_body\" > $sendmail";
      eval { system($str1); };
      my $str2="sh $g_workdir/sendMail.sh \"$mail_subject\"";
      eval { system($str2 . " &"); };
...
sub do_master_failover {
  ...
  eval {
    ...
    #### begin added by cch ########
    $log->info("g_interactive: $g_interactive");
    if ( $g_interactive == 0 ) {
      $log->info("g_logfile: $g_logfile");
    }
    $log->info("g_global_config_file:  $g_global_config_file");
    $log->info("g_config_file:  $g_config_file");
     
    $log->info("g_proxy_config_file: $g_proxy_config_file");
...
#    $dead_master = check_settings($servers_config_ref);
    ...
    #### begin added by cch ############
    if ( $g_interactive == 0 ) { 
      $dead_master = check_settings($servers_config_ref,"$s_ip:$s_port","$g_logfile","$g_config_file");
    }
    else {
      $dead_master = check_settings($servers_config_ref,"$s_ip:$s_port"," ","$g_config_file");
    }
    #### end ###########################
    ...
    $new_master->{activated} = 1;
     ...
     #### begin added by cch #####################
     $log->info("$new_master->get_hostinfo()");
     my $newmaster_addr=$new_master->get_hostinfo();
     $log->info("before  newmaster_addr: $newmaster_addr");
     #$newmaster_addr =~ /\((.*)\)/;
