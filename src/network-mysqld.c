...
void network_mysqld_priv_shutdown(chassis *chas, chassis_private *priv) {
    ...
    for (i = 0; i < len; i++) {
        ...
        g_debug("%s: %p set proxy state ST_PROXY_QUIT", 
                G_STRLOC, con);
    }
}
...
int network_mysqld_init(chassis *srv) {
    ...
    cetus_monitor_register_object(srv->priv->monitor, "users",
                                  cetus_users_reload_callback, srv->priv->users);
...
void network_mysqld_add_connection(chassis *srv, network_mysqld_con *con, gboolean listen)
{
    con->srv = srv;
...
static void cetus_clean_conn_data(network_mysqld_con *con)
{
...
static int network_mysqld_con_send_command(network_socket *con, int cmd, 
        const char *arg) 
{
 ...
 *
 */
int network_mysqld_queue_append_raw(network_socket *sock, 
        network_queue *queue, GString *data) 
{
    guint32 packet_len;
    guint8  packet_id;
        ...
        const char *data, size_t packet_len) 
{
    ...
    do {
        ...
        if (packet_len == PACKET_LEN_MAX) {
            ...
            network_mysqld_proto_append_packet_id(s, ++sock->last_packet_id);
            g_debug("%s: server pack id ++: %d", G_STRLOC, 
                    sock->last_packet_id);
...
int network_mysqld_con_send_ok(network_socket *con) {
    return network_mysqld_con_send_ok_full(con, 0, 0,
        ...
        const gchar *sqlstate, gboolean is_41_protocol) 
{
    ...
    err_packet = is_41_protocol ? network_mysqld_err_packet_new() : 
...
int network_mysqld_con_send_error(network_socket *con, const char *errmsg, 
        gsize errmsg_len) 
{
    return network_mysqld_con_send_error_full(con, errmsg, 
            errmsg_len, ER_UNKNOWN_ERROR, NULL);
}
 ...
 * @return 0 on success
 */
int network_mysqld_con_send_error_pre41_full(network_socket *con, 
        const char *errmsg, gsize errmsg_len, guint errorcode) 
{
    return network_mysqld_con_send_error_full_all(con, errmsg, errmsg_len, 
            errorcode, NULL, FALSE);
}
 ...
 */
int network_mysqld_con_send_error_pre41(network_socket *con, 
        const char *errmsg, gsize errmsg_len) 
{
    return network_mysqld_con_send_error_pre41_full(con, errmsg, 
            errmsg_len, ER_UNKNOWN_ERROR);
}
...
network_mysqld_con_get_packet(chassis *chas, network_socket *con)
{
    ...
    if (!network_queue_peek_str(recv_queue_raw, NET_HEADER_SIZE, 
                &header)) 
    {
    ...
    if (packet_len > chas->cetus_max_allowed_packet) {
        g_message("packet len: %d excess max_allowed_packet: %d",
                  packet_len, chas->cetus_max_allowed_packet);
...
network_mysqld_con_get_uncompressed_packet(chassis *chas, network_socket *con)
{
    ...
    /* read the packet len if the leading packet */
    if (!network_queue_peek_str(con->recv_queue_raw, 
    ...
    /* move the packet from the raw queue to the recv-queue */
    if ((packet = network_queue_pop_str(con->recv_queue_raw, 
                    packet_len + NET_HEADER_SIZE + COMP_HEADER_SIZE, NULL))) 
    {
#if NETWORK_DEBUG_TRACE_IO
        g_debug("%s:output for sock:%p", G_STRLOC, con);
        /* to trace the data we received from the socket, enable this */
        ...
        g_debug("%s: do uncompress here, com len:%d, uncompress len:%d",
                G_STRLOC, packet_len, uncompressed_len);

        GString *uncompressed_packet;
        if (uncompressed_len == 0) {
            uncompressed_len = packet_len;
            uncompressed_packet = g_string_sized_new(uncompressed_len);
            g_string_append_len(uncompressed_packet, (char *) (info + COMP_HEADER_SIZE),
                    uncompressed_len);
        } else {
            uncompressed_packet = g_string_sized_new(uncompressed_len);
            cetus_uncompress(uncompressed_packet, 
                    (unsigned char *) packet->str + header_length, packet_len);
...
static void disp_err_packet(network_mysqld_con *con, network_packet *packet) 
{
    ...
    if (!network_mysqld_proto_get_err_packet(packet, err_packet)) {
        ...
        if (con->dist_tran) {
            ...
            if (!checked) {
                if (strncasecmp(err_packet->sqlstate->str, "XA", 2) == 0) {
                    g_message("%s: query status error, xid:%s for con:%p",
                            G_STRLOC, con->xid_str, con);
                    con->dist_tran_failed = 1;
                    con->xa_query_status_error_and_abort = 1;
                }
            }
        ...
        g_message("%s:clt:%s,src:%s,dst:%s,db:%s,%s",
                ...
                con->server->default_db->str, con->orig_sql->str);
    }
        ...
        network_mysqld_con *con, network_socket *server, int *is_finished)
{
    ...
    *is_finished = 0;
    ...
    if (con->attr_adj_state == ATTR_DIF_CHANGE_USER) {
        ...
        g_debug("%s: set command COM_CHANGE_USER, attr adj:%d for con:%p",
                G_STRLOC, con->attr_adj_state, con);
    ...
    g_debug("%s:after mul read, state:%d, ret:%d for con:%p", 
            G_STRLOC, server->parse.qs_state, ret, con);

    con->parse.command = orig_command;

    if ((*is_finished == 0) && ret == NETWORK_SOCKET_SUCCESS) {
        g_message("%s: not finished for server:%p, orig server:%p", 
                G_STRLOC, con->server, orig_server);
    }
...
network_socket_retval_t network_mysqld_write(chassis G_GNUC_UNUSED *chas, 
        network_socket *sock) 
{
    network_socket_retval_t ret;
...
plugin_call(chassis *srv, network_mysqld_con *con, int state) 
{
    ...
    if (!con->plugin_con_state && con->proxy_state == ST_PROXY_QUIT) {
        g_critical("%s: %p quit because of proxy state not zero", 
                G_STRLOC, con);
        return NETWORK_SOCKET_SUCCESS;
    }
    ...
    case ST_SEND_AUTH_RESULT: 
        ...
        if (!func) {
            ...
            case MYSQLD_PACKET_OK:
                if (con->login_failed) {
                    ...
                    if (con->is_client_compressed) {
                        ...
                        network_socket_set_send_buffer_size(con->client,
                                COMPRESS_BUF_SIZE);
                    }
                }
                break;
            case MYSQLD_PACKET_ERR:
                ...
                break;
            default:
                g_debug("%s: unexpected st for SEND_AUTH_RESULT: %02x",
                        G_STRLOC, con->auth_result_state);
                con->prev_state = con->state;
                con->state = ST_ERROR;
                break;
            }
        ...
        if (!func) { 
            if (!con->server_to_be_closed) {
                if (con->resp_too_long) {
                    g_message("%s: strange here for con:%p",
                        G_STRLOC, con);
                }
                con->state = ST_READ_QUERY;
                g_debug("%s: set ST_READ_QUERY for con:%p",
                        G_STRLOC, con);
            } else {
                con->state = ST_CLOSE_SERVER;
                g_debug("%s: set ST_CLOSE_SERVER for con:%p",
                          G_STRLOC, con);
            }
        }
        break;
    ...
    case ST_ERROR:
        g_debug("%s: not executing plugin func in state ST_ERROR",
                G_STRLOC);
        return NETWORK_SOCKET_SUCCESS;
    default:
...
network_mysqld_con_st_name(network_mysqld_con_state_t state)
{
    ...
    case ST_CONNECT_SERVER: return "ST_CONNECT_SERVER";
    case ST_SEND_HANDSHAKE: return "ST_SEND_HANDSHAKE";
    case ST_READ_AUTH: return "ST_READ_AUTH";
    case ST_SEND_AUTH_RESULT: return "ST_SEND_AUTH_RESULT";
    case ST_READ_QUERY: return "ST_READ_QUERY";
    case ST_GET_SERVER_CONNECTION_LIST: return "ST_GET_SERVER_CONNECTION_LIST";
    case ST_READ_M_QUERY_RESULT: return "ST_READ_M_QUERY_RESULT";
    case ST_SEND_QUERY: return "ST_SEND_QUERY";
    case ST_READ_QUERY_RESULT: return "ST_READ_QUERY_RESULT";
    case ST_SEND_QUERY_RESULT: return "ST_SEND_QUERY_RESULT";

    case ST_CLIENT_QUIT: return "ST_CLIENT_QUIT";
    case ST_CLOSE_CLIENT: return "ST_CLOSE_CLIENT";
    case ST_CLOSE_SERVER: return "ST_CLOSE_SERVER";
    case ST_ERROR: return "ST_ERROR";
    case ST_SEND_ERROR: return "ST_SEND_ERROR";
    }
...
static void 
check_query_status(network_mysqld_con *con, network_socket *server, 
        network_mysqld_com_query_result_t *com_query)
{
    ...
    if (com_query->insert_id > 0) {
        ...
        g_debug("%s: set last insert id:%llu", G_STRLOC, 
                (unsigned long long) con->last_insert_id);
    }
}

void set_conn_attr(network_mysqld_con *con, network_socket *server) 
{
    ...
    case COM_CHANGE_USER:
        g_string_assign_len(server->response->username, 
                            S(con->client->response->username));
    ...
    case COM_SET_OPTION:
        break;
    default:
        g_message("%s: unknown command:%d, con:%p, sql:%s", 
                  G_STRLOC, cur_command,  con, con->orig_sql->str);
        break;
    }
}

void record_xa_log_for_mending(network_mysqld_con *con, network_socket *sock)
{
    ...
    case NEXT_ST_XA_PREPARE:
        tc_log_info(LOG_WARN, 0, "XA END %s %s@%u failed", 
                    con->xid_str, sock->dst->name->str, 
                    sock->challenge->thread_id);
        break;
    ...
    if (con->dist_tran_state <= NEXT_ST_XA_START) {
        ...
        g_warning("%s:xa tran, could not recv response:%p", 
                G_STRLOC, con);
    } else {
        con->dist_tran_failed = 1;
...
shard_set_autocommit(network_mysqld_con *con)
{
    ...
    for (i = 0; i < con->servers->len; i++) {
        ...
        g_queue_push_tail(pmd->server->send_queue->chunks, packet);
...
gboolean shard_set_multi_stmt_consistant(network_mysqld_con *con)
{
    ...
    for (i = 0; i < con->servers->len; i++) {
        ...
        pmd->attr_adjusted_now = 0;
        if ((pmd->attr_diff & ATTR_DIF_SET_OPTION) == 0) {
            continue;
        }
        if (con->client->is_multi_stmt_set != pmd->server->is_multi_stmt_set) {
            ...
            g_queue_push_tail(pmd->server->send_queue->chunks, new_packet);
            g_debug("%s: adjust multi stmt", G_STRLOC);

            pmd->server->is_multi_stmt_set = con->client->is_multi_stmt_set;
            pmd->attr_adjusted_now = 1;
            con->resp_expected_num++;
            result = FALSE;
        } else {
            g_warning("%s:set option adjust warning:not consistant",
                        G_STRLOC);
        }
    }
...
shard_set_charset_consistant(network_mysqld_con *con)
{
    ...
    for (i = 0; i < con->servers->len; i++) {
        ...
        pmd->attr_adjusted_now = 0;
        if ((pmd->attr_diff & ATTR_DIF_CHARSET) == 0) {
            continue;
        }
        ...
        if (strcmp(con->client->charset->str, "") == 0) {
            g_warning("%s: client charset is empty:%s", 
                    G_STRLOC, con->client->src->name->str);
            g_string_append(packet, "''");
            network_mysqld_proto_set_packet_len(packet,
                    1 + strlen(command) + 2);
        } else {
            g_string_append(packet, con->client->charset->str);
            network_mysqld_proto_set_packet_len(packet,
                    1 + strlen(command) + con->client->charset->len);
        }
        ...
        g_debug("%s: adjust default charset for server, clt:%s, srv:%s", 
                G_STRLOC, con->client->charset->str, pmd->server->charset->str);
        pmd->server->parse.qs_state = PARSE_COM_QUERY_INIT;
        con->resp_expected_num++;
...
gboolean shard_set_default_db_consistant(network_mysqld_con *con)
{
    enum enum_server_command command = con->parse.command;
    ...
    for (i = 0; i < con->servers->len; i++) {
        ...
        if ((pmd->attr_diff & ATTR_DIF_DEFAULT_DB) == 0) {
            continue;
        }

        GString *clt_default_db = con->client->default_db;
        GString *srv_default_db = pmd->server->default_db;
        ...
        if (clt_default_db && clt_default_db->len > 0) {
            if (!g_string_equal(clt_default_db, srv_default_db)) {
                ...
                network_mysqld_proto_set_packet_len(new_packet, 
                        1 + clt_default_db->len);
                network_mysqld_proto_set_packet_id(new_packet, 0);
                g_queue_push_tail(pmd->server->send_queue->chunks, new_packet);
                pmd->attr_adjusted_now = 1;
                ...
                g_warning("%s:default db adjust warning:not consistant",
                        G_STRLOC);
            }
        }
    }
...
static session_attr_flags_t next_attribute(session_attr_flags_t flags,
                                           session_attr_flags_t attr)
{
    if (flags == 0 || attr == 0) return 0;
    uint32_t a = attr << 1;
    while (a != 0 && ((a & flags) == 0)) { a <<= 1; }
...
{
    g_debug("%s:build_attr_statements here, attr state:%d",
            G_STRLOC, con->attr_adj_state);

    con->attr_adj_state = next_attribute(con->unmatched_attribute, 
            con->attr_adj_state);
    ...
    case ATTR_START:
        break;
    default:
        g_message("%s:strange attr adj state:%d, conn:%p", 
                  G_STRLOC, con->attr_adj_state, con);
        break;
    }
...
void
shard_build_xa_query(network_mysqld_con *con, server_session_t *pmd)
{
...
static void
build_xa_command(network_mysqld_con *con, server_session_t *pmd, 
        int end, char *buffer_log)
{
    char buffer[XA_CMD_BUF_LEN];
            
    switch(pmd->dist_tran_state) {
    case NEXT_ST_XA_END:
    ...
    case NEXT_ST_XA_PREPARE:
        if (con->servers->len == 1) {
            snprintf(buffer, XA_CMD_BUF_LEN,
                    "XA COMMIT %s ONE PHASE", con->xid_str);
            pmd->dist_tran_state = NEXT_ST_XA_CANDIDATE_OVER;
            con->dist_tran_decided = 1;
        } else {
            snprintf(buffer, XA_CMD_BUF_LEN, "XA PREPARE %s", con->xid_str);
            pmd->dist_tran_state = NEXT_ST_XA_COMMIT;
        }
        if (buffer_log) {
    ...
    case NEXT_ST_XA_ROLLBACK:
        ...
        break;
    default:
        pmd->dist_tran_state = NEXT_ST_XA_OVER;
        pmd->is_xa_over = 1;
        pmd->dist_tran_participated = 0;
        if (end) {
            con->dist_tran_state = NEXT_ST_XA_OVER;
            g_debug("%s: set dist_tran_state NEXT_ST_XA_OVER for con:%p", 
                    G_STRLOC, con);
    ...
    if (end) {
        g_debug("%s: set con dist_tran_state:%d", G_STRLOC, pmd->dist_tran_state);
        con->dist_tran_state = pmd->dist_tran_state;
        con->state = ST_SEND_QUERY;
    }
...
static void 
disp_xa_abnormal_resultset(network_mysqld_con *con, server_session_t *pmd,
        int *is_xa_cmd_met, char **p_buffer, char *buffer, int end) 
{
    if (con->dist_tran_state <= NEXT_ST_XA_QUERY) {
        ...
        build_xa_command(con, pmd, end, NULL);
        ...
        if (*is_xa_cmd_met) {
            ...
            *is_xa_cmd_met = 1;
            build_xa_command(con, pmd, end, *p_buffer);
            *p_buffer = *p_buffer + strlen(*p_buffer);
            (*p_buffer)[0] = ' ';
        }
        (*p_buffer)++;
        snprintf(*p_buffer, XA_BUF_LEN - (*p_buffer - buffer), "%s@%d", 
                pmd->server->dst->name->str, 
                pmd->server->challenge->thread_id);
        *p_buffer = *p_buffer + strlen(*p_buffer);

    } else if (con->dist_tran_state <= NEXT_ST_XA_ROLLBACK) {
        if (pmd->dist_tran_state < con->dist_tran_state) {
            pmd->dist_tran_state = con->dist_tran_state;
            g_message("%s:adjust pmd dist state:%d to %d", G_STRLOC, 
                    pmd->dist_tran_state, con->dist_tran_state);
        }
...
static void 
disp_xa_according_state(network_mysqld_con *con, server_session_t *pmd,
        int *is_xa_cmd_met, int *is_xa_query, char **p_buffer, char *buffer,
        int end) 
{
    if (pmd->dist_tran_state == NEXT_ST_XA_QUERY) {
...
build_xa_statements(network_mysqld_con *con)
{
    ...
    g_debug("%s: call build_xa_statements:%d, server num:%d", 
            G_STRLOC, con->resp_expected_num, len);
    ...
    for (iter = 0; iter < len; iter++) {
        ...
        g_debug("%s: pmd %d, xa state:%d for con:%p", G_STRLOC, iter, 
                pmd->dist_tran_state, con);
        
        if (!con->is_commit_or_rollback && !pmd->participated)  {
            g_debug("%s: stop processing for this server:%d", G_STRLOC, iter);
            continue;
        }

        if (!pmd->dist_tran_participated) {
            continue;
        }

        if (pmd->server->unavailable) {
            g_message("%s: server unavailable and stop processing here:%d", 
                    G_STRLOC, iter);
            continue;
        }
        ...
        if (con->dist_tran_failed) {
            ...
            if (server->recv_queue->chunks->head) {
                if (check_dist_tran_resultset(server->recv_queue, con) == -1) {
                    pmd->xa_query_status_error_and_abort = 1;
                    con->xa_query_status_error_and_abort = 1;
                }
            }
        } else {
            pmd->xa_query_status_error_and_abort = 0;
            ...
            if (result == -1) {
                ...
                con->dist_tran_failed = 1;
                con->xa_query_status_error_and_abort = 1;
                g_message("%s: query status not ok, xid:%llu", 
                        G_STRLOC, con->xa_id);
            }
        ...
        if (result == -1) {
            disp_xa_abnormal_resultset(con, pmd, &is_xa_cmd_met, 
                    &p_buffer, buffer, end);
        } else {
            disp_xa_according_state(con, pmd, &is_xa_cmd_met, 
                    &is_xa_query, &p_buffer, buffer, end);
        }
            
        global_xa_state = pmd->dist_tran_state;
    }
    ...
    if (workers == 0) {
        ...
        if (is_xa_query) {
            ...
            if (con->srv->xa_log_detailed) {
                tc_log_info(LOG_INFO, 0, "XA QUERY %s %s %s",
                        con->xid_str, buffer, con->orig_sql->str);
            }
...
retrieve_error_info_for_xa_trans(network_mysqld_con *con) 
{
    ...
    for (iter = 0; iter < len; iter++) {
        server_session_t *pmd = g_ptr_array_index(con->servers, iter);
        if (!pmd->xa_query_status_error_and_abort) {
            continue;
        }
        ...
        out = pmd->server->recv_queue->chunks;
...
retrieve_one_resp_for_xa_trans(network_mysqld_con *con) 
{
    ...
    for (iter = 0; iter < len; iter++) {
        server_session_t *pmd = g_ptr_array_index(con->servers, iter);
        if (pmd->server->unavailable || !pmd->participated) {
            continue;
        }

        g_debug("%s: retrieve packets for server:%p, index:%d", 
                G_STRLOC, pmd->server, iter);
        out = pmd->server->recv_queue->chunks;
        in = con->client->send_queue;
...
normal_result_merge(network_mysqld_con *con)
{
    ...
    for (i = 0; i < len; i++) {
        server_session_t *pmd = g_ptr_array_index(con->servers, i);
        if (pmd->participated) {
            g_ptr_array_add(recv_queues, pmd->server->recv_queue);
        }
    }
    ...
    resultset_merge(con->client->send_queue, recv_queues,
            con, &uniq_id, &result);
    ...
    case RM_FAIL:
        ...
        if (result.detail) {
            network_mysqld_con_send_error_full(con->client, S(result.detail),
                    ER_CETUS_RESULT_MERGE, "HY000");
            g_string_free(result.detail, TRUE);
        } else {
            network_mysqld_con_send_error_full(con->client, C("merge failed"),
                    ER_CETUS_RESULT_MERGE, "HY000");
        }
        break;
    case RM_CALL_FAIL: {
        char msg[128] = {0};
        snprintf(msg, sizeof(msg), "id:%lu '%s' failed",
                 uniq_id, con->orig_sql->str);
        network_mysqld_con_send_error_full(con->client, msg,
                                           strlen(msg), ER_CETUS_RESULT_MERGE, "HY000");
        break;
    }
    default:
        break;
...
static void disp_query_after_consistant_attr(network_mysqld_con *con) {
    network_socket *recv_sock = con->client;
    GList *chunk = recv_sock->recv_queue->chunks->head;
    GString *packet = (GString *)(chunk->data);
    ...
    for (i = 0; i < con->servers->len; i++) {
        ...
        g_debug("%s:packet id:%d when get server, pmd state:%d", 
                G_STRLOC, pmd->server->last_packet_id, pmd->dist_tran_state);
...
handle_read_query(network_mysqld_con *con, network_mysqld_con_state_t ostate)
{
    ...
    case NETWORK_SOCKET_SUCCESS:
        if (con->retry_serv_cnt > 0 && con->is_wait_server) {
            g_message("%s: wait successful:%d, con:%p",
                      G_STRLOC, con->retry_serv_cnt, con);
    ...
    case NETWORK_SOCKET_ERROR_RETRY:
        if (con->retry_serv_cnt < con->max_retry_serv_cnt) {
            ...
            g_debug(G_STRLOC ": wait again:%d, con:%p, l:%d",
                    con->retry_serv_cnt, con, (int) timeout.tv_usec);
            WAIT_FOR_EVENT(con->client, EV_TIMEOUT, &timeout);
            return DISP_STOP;
        }
        /* fall through */
    default:
        g_critical("%s: wait failed and no server backend for user:%s",
    ...
    if (con->state == ST_SEND_QUERY) {
        ...
        g_debug("%s: call reset_command_response_state for con:%p",
                G_STRLOC, con);
    }
    return  DISP_CONTINUE;
}
...
static void 
process_write_to_server(network_mysqld_con *con, server_session_t *pmd, 
        int *write_wait)
{
    network_socket_retval_t ret;

    con->num_write_pending++;
    ret = network_mysqld_write(con->srv, pmd->server);
    ...
    switch (ret) 
    {
        case NETWORK_SOCKET_SUCCESS:
            ...
            pmd->read_cal_flag = 0;
            g_debug("%s:num_read_pending:%d, pmd->index:%d, reset 0 for con:%p", 
                    G_STRLOC, con->num_read_pending, pmd->index, con);
            con->num_write_pending--;
...
process_shard_write(network_mysqld_con *con, int *disp_flag) 
{
    ...
    for (i = 0; i < con->servers->len; i++) {
        ...
        pmd->index = i;
        pmd->fresh = 0;
        pmd->server->compressed_packet_id = 0;
        pmd->server->resp_len = 0;
        pmd->server->is_read_finished = 0;
        pmd->server->is_waiting = 0;

        if (!pmd->participated || pmd->server->unavailable) {
            g_debug("%s:not participated or unavailable:%d for con%p", G_STRLOC, i, con);
            continue;
        }

        if (con->attr_adj_state != ATTR_START) {
            if (!pmd->attr_adjusted_now) {
        ...
        if (!g_queue_is_empty(pmd->server->send_queue->chunks)) {
            process_write_to_server(con, pmd, &write_wait);
        }
    } /* for each server */
    ...
    if (con->state == ST_READ_M_QUERY_RESULT) {
        ...
        if (con->resp_expected_num != con->num_pending_servers) {
            g_critical("%s:expected resp num:%d, pending:%d, con:%p", 
                    G_STRLOC, con->resp_expected_num, 
                    con->num_pending_servers, con); 
        }
    }
...
static int 
process_rw_write(network_mysqld_con *con, network_mysqld_con_state_t ostate,
        int *disp_flag) 
{
    ...
    switch (con->parse.command) {
        ...
        case COM_STMT_CLOSE:
            ...
            g_debug("%s: conn:%p, sub, now prepare_stmt_count:%d",
                    G_STRLOC, con,
                    con->prepare_stmt_count);
...
static int 
handle_send_query_to_servers(network_mysqld_con *con, 
...
static int 
shard_read_response(network_mysqld_con *con, server_session_t *pmd)
{
    if (pmd->server->resp_len == 0 && pmd->server->to_read == 0) {
        switch(network_socket_to_read(pmd->server)) {
            case NETWORK_SOCKET_SUCCESS:
                if (pmd->server->to_read == 0) {
                    pmd->state = NET_RW_STATE_READ;
    ...
    int is_finished = 0;
    switch (network_mysqld_read_mul_packets(con->srv, con, pmd->server, &is_finished)) {
        case NETWORK_SOCKET_SUCCESS:
            if (is_finished) {
                con->num_pending_servers--;
                ...
                con->num_read_pending++;
                pmd->read_cal_flag = 0;
                g_debug("%s:num_read_pending:%d, pmd->index:%d for con:%p", 
                        G_STRLOC, con->num_read_pending, pmd->index, con);
        ...
        case NETWORK_SOCKET_WAIT_FOR_EVENT:
            if (con->candidate_tcp_streamed) {
                if (!pmd->server->max_header_size_reached) {
                    pmd->state = NET_RW_STATE_READ;
                    server_sess_wait_for_event(pmd, EV_READ, &con->read_timeout);
                } else {
                    if (con->num_servers_visited > 1) {
                        ...
                        g_debug("%s:num read pending:%d for fd:%d", G_STRLOC, 
                                con->num_read_pending, pmd->server->fd);
                ...
                pmd->state = NET_RW_STATE_READ;
                g_debug("%s:num_read_pending:%d for fd:%d, pmd->index:%d", 
                        G_STRLOC, con->num_read_pending, pmd->server->fd, pmd->index);
                server_sess_wait_for_event(pmd, EV_READ, &con->read_timeout);
            }
            break;
        case NETWORK_SOCKET_ERROR:
...
handle_dist_tran_after_read_mul_resp(network_mysqld_con *con, int *result_reserve, int *skip, int *disp_flag)
{
    ...
    if (con->is_timeout || con->is_auto_commit || !con->is_xa_query_sent || 
            con->xa_query_status_error_and_abort) 
    {
        g_debug("%s: call before, con dist tan state:%d for con:%p", 
                G_STRLOC, con->dist_tran_state, con);
        build_xa_statements(con);
        g_debug("%s: call after, con dist tan state:%d for con:%p", 
                G_STRLOC, con->dist_tran_state, con);
        if (con->dist_tran_state != NEXT_ST_XA_PREPARE) {
            if (con->state == ST_SEND_QUERY) {
                ...
                *skip = 1;
                if (con->xa_query_status_error_and_abort) {
                    ...
                    if (con->dist_tran_failed) {
                        ...
                        if (con->dist_tran_state != NEXT_ST_XA_OVER) {
                            g_message("%s: dist state not NEXT_ST_XA_OVER for con:%p",
                                    G_STRLOC, con);
                        }
                        con->dist_tran_state = NEXT_ST_XA_OVER;
                        g_debug("%s: set dist_tran_state NEXT_ST_XA_OVER for con:%p", 
                        ...
                        g_message("%s: send ok to client when timeout:%p",
                                G_STRLOC, con);
                    } else {
                        retrieve_one_resp_for_xa_trans(con);
                    }
                ...
                g_debug("%s: call here for con:%p, xa state:%d", 
                        G_STRLOC, con, con->dist_tran_state);
            }
        } else {
...
read_server_resp(network_mysqld_con *con, int *disp_flag)
{
    ...
    for (i = 0; i < con->servers->len; i++) {
        ...
        if (!pmd->participated || (con->dist_tran && con->xa_start_phase && pmd->xa_start_already_sent)) {
        ...
        if (con->attr_adj_state >= ATTR_DIF_CHANGE_USER) {
            if (pmd->attr_adjusted_now == 0 || pmd->attr_consistent) {
                g_debug("%s: server attr consistent, adj state:%d for con:%p", 
                        G_STRLOC, con->attr_adj_state, con);
                continue;
            }
        }

        if (pmd->state == NET_RW_STATE_FINISHED || pmd->state == NET_RW_STATE_WRITE) {
            g_debug("%s: omit here", G_STRLOC);
            continue;
        }

        if (pmd->state == NET_RW_STATE_ERROR) {
            ...
            if (con->dist_tran) {
                ...
                int j;
                for (j = 0; j < i; j++) {
                    pmd = g_ptr_array_index(con->servers, j);
...
check_server_status(network_mysqld_con *con, int *srv_down_count, int *srv_response_count)
{
    ...
    for (i = 0; i < con->servers->len; i++) {
        ...
        if (pmd->state != NET_RW_STATE_FINISHED && pmd->state != NET_RW_STATE_PART_FINISHED) {
            if (pmd->state == NET_RW_STATE_ERROR) {
                while (con->servers->len > 0) {
                    pmd = g_ptr_array_remove_index_fast(con->servers, 0);
                    network_socket_free(pmd->server);
                    pmd->sql = NULL;
                    pmd->server = NULL;
                    server_session_free(pmd);
                }
...
static void disp_no_workers(network_mysqld_con *con) {
    if (con->dist_tran) {
        network_queue_clear(con->client->send_queue);
        con->dist_tran_state = NEXT_ST_XA_OVER;
        con->dist_tran = 0;
    ...
    con->server_to_be_closed = 1;
    con->state = ST_SEND_QUERY_RESULT;
    g_message("%s: send dist trans error to client, dist state:%d", 
...
static int disp_resp_workers_not_matched(network_mysqld_con *con, int *disp_flag) {
    int result = 1;
            ...
            if (con->xa_query_status_error_and_abort) {
                ...
                if (con->is_attr_adjust) {
                    ...
                    if (con->dist_tran_state < NEXT_ST_XA_CANDIDATE_OVER) {
                        g_debug("%s: build xa stmt for failure, state:%d", 
                                G_STRLOC, con->state);
                        build_xa_statements(con);
                        if (con->dist_tran_state != NEXT_ST_XA_OVER) {
                            *disp_flag = DISP_CONTINUE;
...
{
    server_session_t *pmd = NULL;

    if (con->dist_tran) {
        int iter;
        for (iter = 0; iter < con->servers->len; iter++) {
            server_session_t *candidate_pmd = g_ptr_array_index(con->servers, iter);
            if (candidate_pmd->participated && !candidate_pmd->server->unavailable) {
                pmd = candidate_pmd;
                break;
            }
        }
        ...
        pmd = g_ptr_array_index(con->servers, 0);
    }
...
static int disp_not_skipped(network_mysqld_con *con, int srv_response_count, 
        int *single_response, int *disp_flag) 
{
    switch(con->parse.command) {
        ...
        case COM_QUERY:
            if (srv_response_count > 1) {
                ...
                if (con->partially_merged) {
                    g_debug("%s:partially_merged here:%d", 
                            G_STRLOC, srv_response_count);
                    *disp_flag = DISP_STOP;
                    return 0;
                }
            } else {
...
static void disp_result_not_reserved(network_mysqld_con *con) 
{
    ...
    if (in == NULL || in->length == 0) {
        if (con->dist_tran_decided) {
            ...
            g_warning("%s:send queue empty when sending result to client",
                    G_STRLOC);
            network_mysqld_con_send_error_full(con->client, C("server error"),
...
static int disp_attr(network_mysqld_con *con, int srv_down_count, int *disp_flag)
{
    if (con->num_pending_servers) {
        g_debug("%s: not recv all resp:%d", G_STRLOC, con->state);
        *disp_flag = DISP_STOP;
        ...
        if (con->attr_adj_state <= ATTR_DIF_SET_AUTOCOMMIT) {
            if (srv_down_count > 0) {
                ...
                network_mysqld_con_send_error_full(con->client,
                        C("MySQL server down"), ER_SERVER_SHUTDOWN, "08S01");
...
handle_read_mul_servers_resp(network_mysqld_con *con)
{
    ...
    g_debug("%s:resp count:%d, server num:%d, num pending:%d, state:%d for con:%p", 
            G_STRLOC, srv_response_count, con->servers->len, 
            con->num_pending_servers, con->state, con);
...
static void process_service_unavailable(network_mysqld_con *con) {
    ...
    if (con->servers != NULL) {
        g_debug("%s: server num :%d for con:%p", G_STRLOC, 
                (int) con->servers->len, con);
        size_t i;
        for (i = 0; i < con->servers->len; i++) {
            server_session_t *pmd = g_ptr_array_index(con->servers, i);
            if (pmd->fresh) {
                network_pool_add_idle_conn(pmd->backend->pool, con->srv, pmd->server);
                pmd->backend->connected_clients--;
                g_message("%s: connected_clients sub:%d, %d ndx for con:%p", G_STRLOC, 
                        pmd->backend->connected_clients, (int) i, con);
    ...
    network_mysqld_con_send_error_full(con->client,
            C("service unavailable"), ER_TOO_MANY_USER_CONNECTIONS, "42000");
    con->is_wait_server = 0;
    network_queue_clear(con->client->recv_queue);
    network_mysqld_queue_reset(con->client);
}
...
send_result_to_client(network_mysqld_con *con, network_mysqld_con_state_t ostate)
{
    ...
    if (con->resultset_is_finished) {
        ...
        if (!con->client->is_server_conn_reserved) {
            con->client->is_need_q_peek_exec = 1;
            g_debug("%s: set is_need_q_peek_exec true, state:%d",
                    G_STRLOC, con->state);
        } else {
            con->client->is_need_q_peek_exec = 0;
            g_debug("%s: set is_need_q_peek_exec false",
                    G_STRLOC);
        }
    }
...
normal_read_query_result(network_mysqld_con *con, network_mysqld_con_state_t ostate)
{
    ...
    do {
        ...
        switch (network_mysqld_read(srv, recv_sock)) {
            case NETWORK_SOCKET_SUCCESS:
                ...
                if (recv_sock->resp_len > con->srv->max_resp_len) {
                    ...
                    g_message("%s: resp too long:%p, src port:%s, sql:%s", 
                            G_STRLOC, con, recv_sock->src->name->str,
                            con->orig_sql->str);
                    network_mysqld_con_send_error_full(con->client,
            ...
            case NETWORK_SOCKET_WAIT_FOR_EVENT:
                timeout = con->read_timeout;
                g_debug("%s: set read query timeout, already read:%d", 
                        G_STRLOC, (int) recv_sock->resp_len);
                GString *packet;
            ...
            case NETWORK_SOCKET_ERROR_RETRY:
            case NETWORK_SOCKET_ERROR:
                g_critical("%s: read(READ_QUERY_RESULT) error:%p",
                        G_STRLOC, con);
                con->prev_state = con->state;
                con->state = ST_ERROR;
                break;
        ...
        switch (plugin_call(srv, con, con->state)) {
            case NETWORK_SOCKET_SUCCESS:
                 ...
                 */
                if (!con->resultset_is_finished && 
                        !con->resultset_is_needed) 
                {
                    if (con->client->send_queue->len > 65536) {
                        con->state = ST_SEND_QUERY_RESULT;
                        g_warning("%s: send queue len is too big for sock:%p", 
                                G_STRLOC, con->client);
                    }
                }
                break;
...
read_query_result_for_sharding(network_mysqld_con *con, network_mysqld_con_state_t ostate)
{
    ...
    if (con->resp_too_long) {
        ...
        remove_mul_server_recv_packets(con);
        con->state = ST_SEND_QUERY_RESULT;
        g_message("%s: send server error to client, state:%d", 
                G_STRLOC, con->state);
        network_mysqld_con_send_error(con->client, 
                C("MySQL server connection closed"));
        network_queue_clear(con->client->recv_queue);
        network_mysqld_queue_reset(con->client);
    } else {
        if (con->partially_merged) {
...
process_read_event(network_mysqld_con *con, int event_fd) 
{
    ...
    if (ioctl(event_fd, FIONREAD, &b)) {
        g_critical("ioctl(%d, FIONREAD, ...) failed: %s", 
                event_fd, g_strerror(errno));
        con->prev_state = con->state;
        con->state = ST_ERROR;
    } else if (b != 0) {
                ...
                con->com_quit_seen) 
        {
            ...
            g_message("%s:server closed prematurely, op: %s", 
                    G_STRLOC, network_mysqld_con_st_name(con->state));

            network_mysqld_con_send_error_full(con->client,
                    C("server closed prematurely"), ER_CETUS_UNKNOWN, "HY000");
            con->server_to_be_closed = 1;
            con->server_closed = 1;
            con->resultset_is_finished = TRUE;
...
void network_mysqld_con_handle(int event_fd, short events, void *user_data) {
    ...
    do {
        ...
        case ST_CLOSE_SERVER:
            ...
            plugin_call_cleanup(srv, con);
            g_debug("%s: client conn %p released, state:%d", G_STRLOC, 
                    con, con->state);
        ...
        case ST_CONNECT_SERVER:
            ...
            case NETWORK_SOCKET_SUCCESS:
                ...
                break;
            default:
                g_critical("%s: hook for CONNECT_SERVER invalid: %d", 
                           G_STRLOC, 
                           retval);
        ...
        case ST_SEND_HANDSHAKE: 
            ...
            case NETWORK_SOCKET_SUCCESS:
                break;
            default:
                g_critical("%s: plugin_call(SEND_HANDSHAKE) failed",
                           G_STRLOC);
                con->prev_state = con->state;
                con->state = ST_ERROR;
                break;
        ...
        case ST_READ_AUTH: {
            ...
            case NETWORK_SOCKET_ERROR:
                con->state = ST_ERROR;
                g_message("%s: ST_READ_AUTH: read error for con:%p", 
                          G_STRLOC, con);
                break;
            }
            ...
            case NETWORK_SOCKET_ERROR:
                ...
                break;
            default:
                g_critical("%s: plugin_call(READ_AUTH) failed", 
                           G_STRLOC);
                con->prev_state = con->state;
                con->state = ST_ERROR;
                break;
        ...
        case ST_SEND_AUTH_RESULT: 
            ...
            case NETWORK_SOCKET_ERROR_RETRY:
            case NETWORK_SOCKET_ERROR:
                g_debug("%s: write(AUTH_RESULT) error",
                        G_STRLOC);
                con->prev_state = con->state;
                con->state = ST_ERROR;
                break;
        ...
        case ST_GET_SERVER_CONNECTION_LIST: 
            ...
            case NETWORK_SOCKET_WAIT_FOR_EVENT:
                if (con->retry_serv_cnt < con->max_retry_serv_cnt) {
                    ...
                    struct timeval timeout =
                        network_mysqld_con_retry_timeout(con);
...
static gboolean update_accept_event(network_mysqld_con *con, const int new_flags) {
    g_assert(con != NULL);
        ...
        GPtrArray *rows) 
{
    ...
    for (i = 0; i < fields->len; i++) {
        ...
        network_mysqld_proto_append_lenenc_str(s, 
...
int network_mysqld_con_send_current_date(network_socket *con, const char *name)
{
...
void network_mysqld_send_xa_start(network_socket *sock, const char *xid)
{
...
static retval_t proxy_self_read_handshake(chassis *srv, server_connection_state_t *con)
{
...
static retval_t proxy_self_create_auth(chassis *srv, server_connection_state_t *con)
{
...
process_self_event(server_connection_state_t *con, int events, int event_fd)
{
    if (events == EV_READ) {
        ...
        if (ioctl(con->server->fd, FIONREAD, &b)) {
            g_warning("ioctl(%d, FIONREAD, ...) failed: %s", 
        ...
        if (con->state == ST_ASYNC_CONN) {
            g_message("%s: self conn timeout, state:%d, con:%p, server:%p",
                    G_STRLOC, con->state, con, con->server);
            con->state = ST_ASYNC_ERROR;
...
static int process_self_server_read(server_connection_state_t *con)
{
    ...
    case NETWORK_SOCKET_ERROR:
        g_warning("%s:plugin_call(ASYNC_READ_HANDSHAKE) error",
                  G_STRLOC);
        con->state = ST_ASYNC_ERROR;
        break;
    default:
...
static int process_self_read_auth_result(server_connection_state_t *con)
{
    ...
    if (con->state == ST_ASYNC_ERROR) {
        g_warning("%s: con:%p auth failed,crazy here", 
                G_STRLOC, con);
        return 1;
    }
    ...
    case MYSQLD_PACKET_OK:
        break;
    case MYSQLD_PACKET_ERR:
        g_warning("%s: error send AUTH_RESULT: %02x",
                  G_STRLOC, packet->str[NET_HEADER_SIZE]);
        con->state = ST_ASYNC_ERROR;
        break;
    case MYSQLD_PACKET_EOF:
        con->state = ST_ASYNC_ERROR;
        g_warning("%s: the MySQL 4.0 hash in a MySQL 4.1+ connection",
                  G_STRLOC);
        break;
    default: {
        network_packet pkt;
    ...
    if (con->state != ST_ASYNC_ERROR) {
        ...
        g_debug("%s: connected_clients sub, now:%d for con:%p", 
                G_STRLOC, con->backend->connected_clients, con);
        g_message("%s: backend:%s, new connection:%p",
                G_STRLOC, con->backend->addr->name->str, con->server);
        network_mysqld_queue_reset(con->server);
        network_queue_clear(con->server->recv_queue);
        ...
        void *user_data) 
{
    ...
    do {
        ...
        switch (con->state) {
        case ST_ASYNC_ERROR:
            g_warning("%s: con:%p failed for server:%p", G_STRLOC, 
                      con, con->server);
            con->backend->connected_clients--;
            g_debug("%s: connected_clients sub, now:%d for con:%p", 
                    G_STRLOC, con->backend->connected_clients, con);
        ...
        case ST_ASYNC_CONN:
            ...
            default:
                con->state = ST_ASYNC_ERROR;
                con->backend->state = BACKEND_STATE_DOWN;
                g_message(G_STRLOC ": set backend: %s (%p) down",
                          con->backend->addr->name->str, con->backend);
                break;
            }
            break;
        ...
        if (backend != NULL) {
            ...
            if (backend->state != BACKEND_STATE_UP) {
                if (backend->last_check_time == cur) {
                    g_debug("%s: omit create, backend:%d state:%d", 
                            G_STRLOC, i, backend->state);
                    continue;
                }
            ...
            if (backend->config) {
                if (pool->max_idle_connections > 
                        backend->config->max_conn_pool) 
                {
                    backend->config->max_conn_pool = pool->max_idle_connections;
                    g_message("%s: set max conn pool size:%d", 
                            G_STRLOC, backend->config->max_conn_pool);
            ...
            g_debug("%s: backend ndx:%d total conn:%d, max allowed:%d", 
                    G_STRLOC, i, total, max_allowed_conn_num);

            if (total >= max_allowed_conn_num) {
                g_message("%s: backend ndx:%d reach max conn num:%d", 
                        G_STRLOC, i, max_allowed_conn_num);
                backend->last_check_time = cur;
                continue;
            ...
            g_message("%s: create %s connection for backend ndx:%d, ptr:%p", 
                    G_STRLOC, username, i, backend);
            ...
            if (con->client->default_db && con->client->default_db->len > 0) {
                ...
                g_debug("%s:set server default db:%s for con:%p", 
                        G_STRLOC, scs->server->default_db->str, con);
            }
        ...
        if (backend != NULL) {
            for (j = 0; j < backend->config->mid_conn_pool; j++) {
                        ...
                        backend->config->default_db->len > 0) 
                {
                    ...
                    g_debug("%s:set server default db:%s for con:%p", 
                        G_STRLOC, scs->server->default_db->str, scs);
...
void network_mysqld_con_set_sharding_plan(network_mysqld_con *con, sharding_plan_t *plan)
{
    if (con->sharding_plan) {
        sharding_plan_free(con->sharding_plan);
    }
