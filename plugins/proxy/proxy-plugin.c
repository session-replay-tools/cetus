...
static gboolean proxy_get_backend_ndx(network_mysqld_con *con,
                                      int type, gboolean force_slave);
    ...
    case ST_READ_QUERY_RESULT:
        if (diff < 8 * HOURS) {
            if (con->server && !con->client->is_server_conn_reserved) {
                ...
                g_critical("%s, con:%p read query result timeout, sql:%s",
                           G_STRLOC, con, con->orig_sql->str);

                network_mysqld_con_send_error_full(con->client,
                                                   C("Read query result timeout"),
                                                   ER_ABORTING_CONNECTION, "29001");
                con->prev_state = con->state;
...
static int store_server_ndx_in_prepared_resp(proxy_resultset_t *res, int index)
{
    ...
    switch(inj->id)
    {
        ...
        case INJ_ID_CHANGE_USER:
            if (con->is_changed_user_failed) {
                ...
                network_mysqld_con_send_error_full(con->client,
                    C("Access denied for serving requests"), ER_ACCESS_DENIED_ERROR, "29001");
                ...
                if (!con->is_in_transaction) {
                    if (!con->is_auto_commit) {
                        ...
                        if (con->is_calc_found_rows) {
                            con->client->is_server_conn_reserved = 1;
                            g_debug("%s: set is_server_conn_reserved true for con:%p",
                                    G_STRLOC, con);
                        }  else {
...
static int network_mysqld_con_handle_insert_id_response(network_mysqld_con *con,
        const char *name, int last_packet_id)
{
    char buffer[16];
    GPtrArray *fields;
    GPtrArray *rows;
...
static int process_trans_query(network_mysqld_con *con)
{
    proxy_plugin_con_t *st = con->plugin_con_state;
    sql_context_t *context = st->sql_context;
...
static int process_non_trans_query(network_mysqld_con *con,
        sql_context_t *context, mysqld_query_attr_t *query_attr)
{
    ...
    case STMT_SELECT: {
        ...
        if (is_insert_id == TRUE) {
            g_debug("%s: buffered last insert id:%d", G_STRLOC,
                    (int) con->last_insert_id);
            network_mysqld_con_handle_insert_id_response(con,
                          last_insert_id_name, con->last_insert_id);
            ...
            if (expr && expr->op == TK_EQ) {
                ...
                if (sql_context_is_autocommit_off(context)) {
                    ...
                    if (left->op == TK_ID && right->token_text) {
                        process_other_set_command(con, left->token_text,
                                                  right->token_text, query_attr);
                    }
                }
            }
        }
    ...
    if (con->srv->master_preferred || context->rw_flag & CF_WRITE
            || !con->is_auto_commit)
    {
        /* rw operation */
...
static void
proxy_inject_packet(network_mysqld_con *con, int type, int resp_type,
        GString *payload, gboolean resultset_is_needed)
{
    proxy_plugin_con_t *st = con->plugin_con_state;
...
static int change_stmt_id(network_mysqld_con *con, uint32_t stmt_id) {
    proxy_plugin_con_t *st = con->plugin_con_state;
...
static int change_server_by_rw(network_mysqld_con *con, int backend_ndx)
{
    if (backend_ndx >= 0) {
        proxy_plugin_con_t *st = con->plugin_con_state;
        int index = st->backend_ndx_array[backend_ndx] - 1;
        g_debug("conn:%p, change_server_by_rw,ndx:%d, index:%d, st ndx:%d",
    ...
    if (!query_attr->sql_mode_set) {
        if (strcasecmp(clt_sql_mode, srv_sql_mode) != 0) {
            if (strcmp(clt_sql_mode, "") != 0) {
                ...
                g_string_append_len(packet, con->client->sql_mode->str,
                        con->client->sql_mode->len);
                g_string_append(packet, "'");
                proxy_inject_packet(con, PROXY_QUEUE_ADD_PREPEND,
                        INJ_ID_CHANGE_SQL_MODE, packet, TRUE);
            } else {
                GString *packet = g_string_new(NULL);
                g_string_append_c(packet, (char) COM_QUERY);
                g_string_append(packet, "SET sql_mode=''");
            ...
            g_string_assign_len(con->server->sql_mode,
                    con->client->sql_mode->str,
                    con->client->sql_mode->len);
        }
    } else {
        g_string_assign_len(con->server->sql_mode,
...
adjust_charset(network_mysqld_con *con, mysqld_query_attr_t *query_attr)
{
    ...
    if (!query_attr->charset_set) {
        if (!g_string_equal(con->client->charset, con->server->charset)) {
            ...
            if (charset->len > 0) {
                ...
                g_string_assign_len(con->server->charset_client,
                                    charset->str, charset->len);
                g_string_assign_len(con->server->charset_connection,
                                    charset->str, charset->len);
                g_string_assign_len(con->server->charset_results,
                                    charset->str, charset->len);
            }
            g_string_assign_len(con->server->charset, charset->str,
                                charset->len);
        }
    } else {
        GString *charset = con->client->charset;
    ...
    if (!query_attr->charset_client_set) {
        if (!g_string_equal(con->client->charset_client, con->server->charset_client)) {
            if (con->client->charset_client->len > 0) {
                ...
                proxy_inject_packet(con, PROXY_QUEUE_ADD_PREPEND,
                        INJ_ID_CHAR_SET_CLT, packet, TRUE);
            }
    ...
    if (!query_attr->charset_connection_set) {
        if (!g_string_equal(con->client->charset_connection,
                            con->server->charset_connection)) {
            if (con->client->charset_connection->len > 0) {
                ...
                proxy_inject_packet(con, PROXY_QUEUE_ADD_PREPEND,
                        INJ_ID_CHAR_SET_CONN, packet, TRUE);
            }
    ...
    if (!query_attr->charset_results_set) {
        if (!g_string_equal(con->client->charset_results, con->server->charset_results)) {
            if (con->client->charset_results->len > 0) {
                ...
                proxy_inject_packet(con, PROXY_QUEUE_ADD_PREPEND,
                        INJ_ID_CHAR_SET_RESULTS, packet, TRUE);
            } else {
                GString *packet = g_string_new(NULL);
                g_string_append_c(packet, (char) COM_QUERY);
...
adjust_default_db(network_mysqld_con *con, enum enum_server_command cmd)
{
    ...
    if (clt_default_db->len > 0) {
        if (!g_string_equal(clt_default_db, srv_default_db)) {
            ...
            g_string_append_len(packet, clt_default_db->str,
                    clt_default_db->len);
            proxy_inject_packet(con, PROXY_QUEUE_ADD_PREPEND,
                    INJ_ID_CHANGE_DB, packet, TRUE);
...
reset_connection(network_mysqld_con *con)
{
    ...
    proxy_inject_packet(con, PROXY_QUEUE_ADD_PREPEND,
            INJ_ID_RESET_CONNECTION, packet, TRUE);
...
adjust_user(network_mysqld_con *con)
{
    g_debug("%s: user:%s try to robs conn from user:%s, server:%p for con:%p", G_STRLOC,
            con->client->response->username->str, con->server->response->username->str,
            con->server, con);
    ...
    if (hashed_password->len == 0) {
        ...
        proxy_inject_packet(con, PROXY_QUEUE_ADD_PREPEND,
                    INJ_ID_CHANGE_USER, payload, TRUE);
...
adjust_multi_stmt(network_mysqld_con *con, enum enum_server_command cmd)
{
    if (con->client->is_multi_stmt_set != con->server->is_multi_stmt_set) {
        ...
        g_string_append_c(packet, (char) 0);
        proxy_inject_packet(con, PROXY_QUEUE_ADD_PREPEND,
                INJ_ID_CHANGE_MULTI_STMT, packet, TRUE);
...
gboolean network_mysqld_con_is_trx_feature_changed(network_mysqld_con *con)
{
    proxy_plugin_con_t *st = con->plugin_con_state;
    if (!st) {
        return FALSE;
    }
    return st->trx_read_write != TF_READ_WRITE
        || st->trx_isolation_level != TF_REPEATABLE_READ;
}

void network_mysqld_con_reset_trx_feature(network_mysqld_con *con)
{
    proxy_plugin_con_t *st = con->plugin_con_state;
    if (st) {
...
static int proxy_handle_local_query(network_mysqld_con *con, sql_context_t *context)
{
...
process_quit_cmd(network_mysqld_con *con, int backend_ndx, int *disp_flag)
{
    if (backend_ndx < 0 || 
            (!con->is_in_transaction && 
             !network_mysqld_con_is_trx_feature_changed(con))) 
    {
...
static int forced_visit(network_mysqld_con *con, proxy_plugin_con_t *st,
        sql_context_t *context, int *disp_flag)
{
...
static int process_rw_split(network_mysqld_con *con, proxy_plugin_con_t *st,
        sql_context_t *context, mysqld_query_attr_t *query_attr, 
        int is_under_sess_scope, int command, int *disp_flag)
{
    if (!con->is_in_transaction && !is_under_sess_scope && command == COM_QUERY)
    {
        ...
        if (con->is_in_transaction) {
            ...
            } else if (con->prepare_stmt_count > 0 ||
                    !con->is_auto_commit) {
                query_attr->conn_reserved = 1;
...
process_query_or_stmt_prepare(network_mysqld_con *con, proxy_plugin_con_t *st,
        network_packet *packet, mysqld_query_attr_t *query_attr, 
        int command, int *disp_flag)
{
    ...
    switch (context->stmt_type) {
        case STMT_SELECT:stats->com_select += 1;break;
        case STMT_UPDATE:stats->com_update += 1;break;
        case STMT_INSERT:stats->com_insert += 1;break;
        case STMT_DELETE:stats->com_delete += 1;break;
        default:break;
    }
    ...
    if (context->rw_flag & (CF_FORCE_MASTER|CF_FORCE_SLAVE)) {
        ...
        if (context->stmt_type == STMT_SET_TRANSACTION) {
            ...
            if (feat->scope == SCOPE_SESSION) {
                g_message("%s:set session transaction sql:%s for con:%p",
                ...
                if (feat->rw_feature) {
                    ...
                    g_warning("%s:unexpected transaction feature:%s",
        ...
        /* rw split */
        if (!process_rw_split(con, st, context, query_attr, is_under_sess_scope,
                    command, disp_flag))
        {
            return 0;
        }
...
static network_mysqld_stmt_ret network_read_query(network_mysqld_con *con,
        proxy_plugin_con_t *st)
{
    ...
    case COM_BINLOG_DUMP:
        network_mysqld_con_send_error(con->client,
               C("(proxy) unable to process binlog dump"));
        return PROXY_SEND_RESULT;
    case COM_QUERY:
    case COM_STMT_PREPARE:
        if (!process_query_or_stmt_prepare(con, st, &packet, &query_attr,
                    command, &disp_flag))
        {
            return disp_flag;
        }
    ...
    if (backend->state != BACKEND_STATE_UP && backend->state != BACKEND_STATE_UNKNOWN) {
        ...
        case COM_QUERY: {
            network_mysqld_con_send_error_full(con->client,
                                               C("proxy stops serving requests now"),
                                               ER_ABORTING_CONNECTION, "29001");
    ...
    switch(command) {
    case COM_QUERY:
        proxy_inject_packet(con, PROXY_QUEUE_ADD_APPEND,
                            INJ_ID_COM_QUERY, payload, TRUE);
        break;
    case COM_STMT_PREPARE:
        proxy_inject_packet(con, PROXY_QUEUE_ADD_APPEND,
                            INJ_ID_COM_STMT_PREPARE, payload, TRUE);
        break;
    default:
        proxy_inject_packet(con, PROXY_QUEUE_ADD_APPEND,
                            INJ_ID_COM_DEFAULT, payload, TRUE);
    }
    ...
    if (con->rob_other_conn) {
        ...
        if (adjust_user(con) == -1) {
            network_injection_queue_reset(st->injected.queries);
            network_mysqld_con_send_error_full(con->client,
                                               C("proxy stops serving requests"),
                                               ER_NO_SUCH_USER, "29001");
...
NETWORK_MYSQLD_PLUGIN_PROTO(proxy_read_query) {
    ...
    case PROXY_NO_DECISION:
        if (st->injected.queries->length) {
            g_critical("%s: discarding %d elements from the queue.",
                       G_STRLOC,
    ...
    case PROXY_SEND_RESULT: {
        ...
        while ((packet = g_queue_pop_head(chunks))) {
            if (is_first_packet) {
                ...
                g_debug("%s: call network_mysqld_con_command_states_init for con:%p",
                        G_STRLOC, con);
    ...
    case PROXY_SEND_INJECTION: {
        ...
        network_mysqld_queue_append(send_sock,
                                    send_sock->send_queue, S(inj->query));
...
NETWORK_MYSQLD_PLUGIN_PROTO(proxy_send_query_result) {
    ...
    if (!inj->resultset_is_needed && st->injected.sent_resultset > 0) {
         ...
         */
        g_critical("%s: append() mul-queries without true rs set.",
                G_STRLOC);
...
NETWORK_MYSQLD_PLUGIN_PROTO(proxy_read_query_result) {
    ...
    g_debug("%s: here we visit network_mysqld_proto_get_query_result for con:%p",
            G_STRLOC, con);
                ...
                case COM_STMT_EXECUTE:
                   {
                        ...
                        if (!com_query->was_resultset) {
                            ...
                            if (inj->qstat.insert_id > 0) {
                                ...
                                g_debug("%s: last insert id:%d for con:%p",
                                        G_STRLOC, (int) con->last_insert_id, con);
                            }
                        }
...
static proxy_plugin_con_t *proxy_plugin_con_new()
{
    proxy_plugin_con_t *st;
...
proxy_c_disconnect_client(network_mysqld_con *con)
{
    ...
    if (client_abnormal_close) {
        ...
        if (con->is_in_transaction || !con->is_auto_commit || con->is_in_sess_context)
        {
            ...
            if (con->server != NULL) {
                if (network_pool_add_conn(con, 0) != 0) {
                    g_debug("%s, con:%p:conn returned to pool failed",
                            G_STRLOC, con);
                }
            }
        }
...
static void mysqld_con_reserved_connections_free(network_mysqld_con *con)
{
    proxy_plugin_con_t *st = con->plugin_con_state;
    chassis *srv = con->srv;
...
NETWORK_MYSQLD_PLUGIN_PROTO(proxy_disconnect_client) {
    ...
    if (con->server && !con->server_to_be_closed) {
        if (con->state == ST_CLOSE_CLIENT || con->prev_state <= ST_READ_QUERY)
        {
            ...
            if (network_pool_add_conn(con, 0) != 0) {
                g_message("%s, con:%p:server conn returned to pool failed",
                        G_STRLOC, con);
            }
        }
    }
...
void network_mysqld_proxy_plugin_free(chassis *chas, chassis_plugin_config *config) {
...
network_mysqld_proxy_plugin_get_options(chassis_plugin_config *config)
{
    ...
    chassis_options_add(&opts, "read-master-percentage",
        0, 0, OPTION_ARG_INT, &(config->read_master_percentage),
        "range [0, 100]", NULL);
 ...
 */
int network_mysqld_proxy_plugin_apply_config(chassis *chas,
        chassis_plugin_config *config)
{
...
GList *network_mysqld_proxy_plugin_allow_ip_get(chassis_plugin_config *config) {
...
GList *network_mysqld_proxy_plugin_deny_ip_get(chassis_plugin_config *config) {
...
gboolean network_mysqld_proxy_plugin_allow_ip_add(chassis_plugin_config *config, char *addr) {
...
gboolean network_mysqld_proxy_plugin_deny_ip_add(chassis_plugin_config *config, char *addr) {
    if (!config || !addr) return FALSE;
...
gboolean network_mysqld_proxy_plugin_allow_ip_del(chassis_plugin_config *config, char *addr) {
    if (!config || !addr || !config->allow_ip_table) return FALSE;
    return g_hash_table_remove(config->allow_ip_table, addr);
}

gboolean network_mysqld_proxy_plugin_deny_ip_del(chassis_plugin_config *config, char *addr) {
