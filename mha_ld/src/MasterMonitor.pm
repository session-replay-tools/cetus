...
my $g_seconds_behind_master = 7201;
#### end ####################################
...
sub wait_until_master_is_unreachable() {
  ...
  eval {
   ...
   #### begin added by cch ##########################################################
    ...
    $log->info("proxy_conf: $g_proxy_config_file");
    #### end #########################################################################
...
"$_status_handler->{status_file} already exists. You might have killed manager with SIGKILL(-9), may run two or more monitoring process for the same application, or use the same working directory. Check for details, and consider setting --workdir separately."
    ...
    if ( !$_server_manager->is_gtid_auto_pos_enabled() ) {
      ...
      $log->info("g_workdir: $g_workdir");
      $log->info("------------------SSHCheck::do_ssh_connection_check-------------------------");
...
"GTID (with auto-pos) is supported. Skipping all SSH and Node package checking."
  ...
  eval {
    ...
    $master_ping = new MHA::HealthCheck(
      ...
      logfile                => $g_logfile,
      workdir                => $g_workdir,
      proxy_conf             => $g_proxy_config_file,         #### begin added by cch #####
...
sub main {
  ...
  GetOptions(
    ...
    'proxy_conf=s'            => \$g_proxy_config_file, #### begin added by cch ########
  ...
  while (1) {
    ...
    my $gt = gettime;
    if ($exit_code) {
      send_alert("$gt MHA ".$msg.$g_config_file);
    }
    ####end ###############################################
