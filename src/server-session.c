...
void server_sess_wait_for_event(server_session_t *pmd, short ev_type,
...
static void do_tcp_stream_after_recv_resp(network_mysqld_con *con, server_session_t *pmd) 
{
    ...
    if (con->server_closed) {
        ...
        network_mysqld_con_send_error_full(con->client,
                C("server closed prematurely"), ER_CETUS_RESULT_MERGE, "HY000");
...
static int process_read_event(network_mysqld_con *con, server_session_t *pmd)
{
    ...
    if ((i = ioctl(sock->fd, FIONREAD, &b))) {
        ...
        g_message("%s:NET_RW_STATE_ERROR is set i:%d,b:%d", 
                G_STRLOC, i, b);
    } else if (b >= 0) {
        sock->to_read = b;
        sock->resp_len += b;
...
static void process_timeout_event(network_mysqld_con *con, server_session_t *pmd)
{
    network_socket *sock = pmd->server;
    if (con->dist_tran) {
        record_xa_log_for_mending(con, sock);
...
process_read_server(network_mysqld_con *con, server_session_t *pmd)
{
    ...
    if (sock->to_read == 0) {
        ...
        ret = network_mysqld_read_mul_packets(con->srv, con, 
                sock, &is_finished);
    }

    switch (ret) {
        case NETWORK_SOCKET_SUCCESS:
            g_debug("%s: resp len:%d, attr adj state:%d", 
                    G_STRLOC, (int) sock->resp_len, con->attr_adj_state);
        ...
        case NETWORK_SOCKET_WAIT_FOR_EVENT:
            if (sock->resp_len > con->srv->max_resp_len) {
                ...
                con->server_to_be_closed = 1;
                con->resp_too_long = 1;
                g_debug("%s: resp len is too long for query:%s",
                ...
                if (con->candidate_tcp_streamed && sock->max_header_size_reached && 
                        con->num_servers_visited > 1) 
                {
                    ...
                    if (con->num_servers_visited == 1 && sock->max_header_size_reached
                            && con->candidate_tcp_streamed) 
                    {
                        GString *packet;
                        while ((packet = g_queue_pop_head(pmd->server->recv_queue->chunks)) != NULL) {
                            network_mysqld_queue_append_raw(con->client, con->client->send_queue, packet);
...
void server_session_con_handler(int event_fd, short events, void *user_data) 
{
    ...
    pmd->read_cal_flag = 0;
    g_debug("%s:visit server_session_con_handler, state:%d for con:%p, fd:%d", 
            G_STRLOC, con->state, con, sock->fd);
    ...
    g_debug("%s:server_session_con_handler over for con:%p, pmd:%d", 
            G_STRLOC, con, pmd->index);
}
