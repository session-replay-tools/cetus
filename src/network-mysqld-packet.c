 ...
 */
int network_mysqld_proto_get_com_query_result(network_packet *packet, 
    ...
    switch (query->state) {
        case PARSE_COM_QUERY_INIT:
            ...
            switch (status) {
                ...
                case MYSQLD_PACKET_EOF:
                    g_critical("%s: COM_QUERY should not be (EOF), got: 0x%02x",
                            G_STRLOC,
                            status);
                    err = 1;
                    break;
        ...
        case PARSE_COM_QUERY_FIELD:
            ...
            switch (status) {
                ...
                case MYSQLD_PACKET_NULL:
                    g_critical("%s: COM_QUERY !=(OK|NULL|ERR), got: 0x%02x",
                            G_STRLOC,
                            status);
                ...
                case MYSQLD_PACKET_EOF:
                    ...
                    if (packet->data->len == 9) {
                        ...
                        err = network_mysqld_proto_get_eof_packet(packet, 
                                eof_packet);
                        ...
                        if (!err) {
                            ...
                            if (use_binary_row_data &&
                                ...
                                is_finished = 1;
                                g_message("%s: is_finished here without PARSE_COM_QUERY_RESULT",
                                        G_STRLOC);
                            } else {
                            ...
                            query->state = PARSE_COM_QUERY_RESULT;
#endif
                            g_debug("%s: set query state:%d for parse.data:%p",
                                    G_STRLOC, query->state, query);
...
int network_mysqld_proto_get_com_stmt_prep_result(
    ...
    if (udata->first_packet == 1) {
        ...
        switch (status) {
            case MYSQLD_PACKET_OK:
                ...
                if (packet->data->str[NET_HEADER_SIZE + 5] != 0 || 
                        packet->data->str[NET_HEADER_SIZE + 6] != 0) 
                {
                    udata->want_eofs++;
                }
                if (packet->data->str[NET_HEADER_SIZE + 7] != 0 ||
                        packet->data->str[NET_HEADER_SIZE + 8] != 0) 
                {
                    udata->want_eofs++;
                }
            ...
            case MYSQLD_PACKET_ERR:
                is_finished = 1;
                g_message("%s: network_mysqld_proto_get_com_stmt_prep_rs get packet err:%d",
                        G_STRLOC,
                        status);
                udata->status = status;
                break;
            default:
                g_error("%s: COM_STMT_PREPARE should either get a (OK|ERR), got %02x",
                        G_STRLOC,
                        status);
                break;
        }
        ...
        switch (status) {
            ...
            case MYSQLD_PACKET_NULL:
            case MYSQLD_PACKET_ERR:
                g_error("%s: COM_STMT_PREPARE should not be (OK|ERR|NULL), got: %02x",
                        G_STRLOC,
                        status);
                break;
            case MYSQLD_PACKET_EOF:
...
int network_mysqld_com_init_db_result_track_state(network_packet *packet, 
        network_mysqld_com_init_db_result_t *udata) 
{
    ...
    if (packet->offset != packet->data->len) {
        ...
        network_mysqld_proto_get_gstr_len(packet, 
                packet->data->len - packet->offset, udata->db_name);
    } else {
        if (udata->db_name) g_string_free(udata->db_name, TRUE);
        udata->db_name = NULL;
    }
    ...
    switch (status) {
        case MYSQLD_PACKET_ERR:
            ...
            if (udata->db_name && udata->db_name->len) {
                ...
                g_message("%s: COM_INIT_DB failed, client default db still:%s", 
        ...
        case MYSQLD_PACKET_OK:
            ...
            if (udata->db_name && udata->db_name->len) {
                ...
                if (con->server) {
                    ...
                    g_debug("%s:set server default db:%s for con:%p", 
                        G_STRLOC, con->server->default_db->str, con);
                }

                g_string_append_len(con->client->default_db, 
                        S(udata->db_name));
                g_debug("%s: COM_INIT_DB set default db success:%s", 
                        G_STRLOC, con->client->default_db->str);
            } else {
                if (con->server) {
                    g_string_append_len(con->server->default_db, 
                            S(con->client->default_db));
                }
            }
            ...
            break;
        default:
            g_critical("%s: COM_INIT_DB should be (ERR|OK), got %02x",
                    G_STRLOC,
                    status);
...
int network_mysqld_proto_get_query_result(network_packet *packet, network_mysqld_con *con) {
    ...
    switch (con->parse.command) {
        ...
        case COM_CHANGE_USER: 
            ...
            switch (status) {
                case MYSQLD_PACKET_ERR:
                    ...
                    if (!network_mysqld_proto_get_err_packet(packet, err_packet)) {
                        g_warning("%s:error code:%d,errmsg:%s,sqlstate:%s",
                                G_STRLOC, (int) err_packet->errcode,
                ...
                case MYSQLD_PACKET_OK:
                    g_string_assign_len(con->server->response->username, 
                ...
                case MYSQLD_PACKET_EOF:
                     ...
                     */
                    g_message("%s: COM_CHANGE_USER's auth-method-switch not supported.",
                            G_STRLOC);
                    return -1;
                default:
                    g_debug_hexdump(G_STRLOC, S(packet->data));
    ...
    case COM_STATISTICS:
        ...
        break;
    case COM_STMT_PREPARE:
        is_finished = network_mysqld_proto_get_com_stmt_prep_result(packet,
                                                                    con->parse.data);
        break;
    case COM_STMT_EXECUTE:
         ...
         */
        is_finished = network_mysqld_proto_get_com_query_result(packet,
                                                                con->parse.data, TRUE);
        break;
    case COM_PROCESS_INFO:
    case COM_QUERY:
        is_finished = network_mysqld_proto_get_com_query_result(packet, 
                                                                con->parse.data, FALSE);
        break;
...
int network_mysqld_proto_get_fielddef(network_packet *packet, 
        network_mysqld_proto_fielddef_t *field, guint32 capabilities) 
{
    int err = 0;
        ...
        network_mysqld_ok_packet_t *ok_packet) 
{
    ...
    if (!err) {
        ...
        g_debug("%s: server status, got: %d", G_STRLOC,
                ok_packet->server_status);
    }
...
int network_mysqld_proto_append_ok_packet(GString *packet, 
        network_mysqld_ok_packet_t *ok_packet) 
{
    guint32 capabilities = CLIENT_PROTOCOL_41;
 ...
 */
int network_mysqld_proto_get_err_packet(network_packet *packet, 
        network_mysqld_err_packet_t *err_packet) 
{
    ...
    if (packet->offset < packet->data->len) {
        err = err || network_mysqld_proto_get_str_len(packet, 
                &errmsg, packet->data->len - packet->offset);
    }
 ...
 * @return 0 on success
 */
int network_mysqld_proto_append_err_packet(GString *packet, 
        network_mysqld_err_packet_t *err_packet) 
{
    int errmsg_len;
 ...
 * decode a OK packet from the network packet
 */
int network_mysqld_proto_get_eof_packet(network_packet *packet, 
        network_mysqld_eof_packet_t *eof_packet) 
{
    guint8 field_count;
...
void network_mysqld_auth_challenge_free(network_mysqld_auth_challenge *shake) {
    ...
    if (shake->scrambled_password) g_string_free(shake->scrambled_password, TRUE);
...
int network_mysqld_proto_get_auth_challenge(network_packet *packet, 
        network_mysqld_auth_challenge *shake) 
{
    int maj, min, patch;
    ...
    if (!err) {
        ...
        if (shake->capabilities & CLIENT_PLUGIN_AUTH) {
            ...
            err = network_mysqld_proto_get_str_len(packet, &auth_plugin_data_2, 
                    auth_plugin_data2_len);
    ...
    if (!err) {
        ...
        if (shake->capabilities & CLIENT_PLUGIN_AUTH) {
            g_string_assign_len(shake->auth_plugin_data, auth_plugin_data_1, 
                    MIN(8, auth_plugin_data_len));
            if (auth_plugin_data_len > 8) {
                g_string_append_len(shake->auth_plugin_data, auth_plugin_data_2, 
...
int network_mysqld_proto_append_auth_challenge(GString *packet, 
        network_mysqld_auth_challenge *shake) 
{
    guint i;
...
int network_mysqld_proto_get_auth_response(network_packet *packet, 
        network_mysqld_auth_response *auth) 
{
    int err = 0;
    guint16 l_cap;
...
int network_mysqld_proto_get_and_change_auth_response(network_packet *packet, 
        network_mysqld_auth_response *auth) 
{
    int err = 0;
 ...
 */
int network_mysqld_proto_append_auth_response(GString *packet, 
        network_mysqld_auth_response *auth) 
{
    if (!(auth->client_capabilities & CLIENT_PROTOCOL_41)) {
        ...
        g_string_append_len(packet, auth->auth_plugin_data->str, 
                            auth->auth_plugin_data->len & 0xff);
...
int network_mysqld_proto_get_stmt_id(network_packet *packet, guint32 *stmt_id)
{
    guint8 packet_type;
    int err = 0;
...
int network_mysqld_proto_change_stmt_id_from_ok_packet(network_packet *packet,
        int server_index) 
{
    guint8 packet_type;
    int err = 0;
    ...
    *p = *p | (server_index << 16);
    ...
    g_debug("%s: new stmt id:%d, server index:%d",
            G_STRLOC,
            *p,
            server_index);
...
int network_mysqld_proto_change_stmt_id_from_clt_stmt(network_packet *packet) {
    guint8 packet_type;
    int err = 0;
    int *p = NULL;
...
int network_mysqld_proto_append_query_packet(GString *packet, const char *query)
{
...
int mysqld_proto_append_change_user_packet(GString *packet,
                                           mysqld_change_user_packet_t *chuser)
{
    ...
    if (!chuser->auth_plugin_data) {
        ...
        network_mysqld_proto_password_scramble(auth_response,
                    S(chuser->auth_plugin_data), S(chuser->hashed_pwd));
