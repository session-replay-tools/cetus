...
"Failed to get current master. Maybe replication is misconfigured or configuration file is broken?"
  ...
  #### begin added by cch #########################################
  #$log->info("orig_master: $orig_master");
  #$log->info("orig_master: $orig_master->get_hostinfo()");
  
  $log->info("g_config_file: $g_config_file");
  $log->info( sprintf( "orig_master: %s", $orig_master->get_hostinfo() ) );
  my $addr = $orig_master->get_hostinfo();
  $addr =~ /\((.*)\)/; 
...
"Getting advisory lock failed on the current master. MHA Monitor runs on the current master. Stop MHA Manager/Monitor and try again."
    ...
    ## setproxy failover unknown rw hostinfo    
...
sub switch_slaves($$$$$$) {
  ...
  if ($g_orig_master_is_new_slave) {
      ...
      )
    {
      ...
      my $orig_master_addr=$orig_master->get_hostinfo();
      $log->info("before  orig_master_addr: $orig_master_addr");
...
sub do_master_online_switch {
  ...
  eval {
    ...
    $log->info( sprintf( "new_master: %s", $new_master->get_hostinfo() ) ); 
    $log->info("g_config_file: $g_config_file ");
...
sub main {
  ...
  ####begin added by cch #######################################
  print "new_master_host: $g_new_master_host \n" ;  
  print "new_master_port : $g_new_master_port \n" ;
